...
strictify I       = ≅.refl
strictify (X ⊗ Y) =
  strictify X ⊗ᵢ strictify Y  ≅⟨ eval (nf X) ⊗₀ eval (nf Y) ⟩
  ≅.sym (eval-homo (nf X) (nf Y))
...
invert-isoˡ (g ⊚ f)  = Cancellers.cancelInner (invert-isoˡ g) ○ invert-isoˡ f
...
invert-isoʳ (g ⊚ f)  = Cancellers.cancelInner (invert-isoʳ f) ○ invert-isoʳ g
...
private
  ...
  assoc-shuffle = begin
    α⇒ ∘ (α⇐ ⊗₁ id) ∘ α⇐                                 ≈˘⟨ refl⟩∘⟨ identityʳ ⟩
    α⇒ ∘ ((α⇐ ⊗₁ id) ∘ α⇐) ∘ id                          ≈˘⟨ refl⟩∘⟨ refl⟩∘⟨ ⊗-cancel identity² associator.isoˡ ⟩
    α⇒ ∘ ((α⇐ ⊗₁ id) ∘ α⇐) ∘ ((id ⊗₁ α⇐) ∘ (id ⊗₁ α⇒))   ≈⟨ refl⟩∘⟨ sym-assoc ⟩
...
substₑ-∘ : {x y z : List Atom} (p : x ≡ y) (q : y ≡ z)
  → substₑ (≡.trans p q) ≈ substₑ q ∘ substₑ p
...
κ⁻¹-natural (λ⇒ᶠ {X}) = begin
  ...
  (κ⁻¹ X ∘ λ⇒) ∘ λ⇐           ≈⟨ Cancellers.cancelʳ unitorˡ.isoʳ ⟩
  κ⁻¹ X                       ≈˘⟨ identityʳ ⟩
  κ⁻¹ X ∘ id                  ∎
κ⁻¹-natural (λ⇐ᶠ {X}) = begin
...
κ⁻¹-natural (α⇐ᶠ {A} {B} {C}) = ⟺ (begin
  ...
  (α⇐ ∘ (κ⁻¹ (A ⊗ (B ⊗ C)) ∘ assocₑ A B C)) ∘ assocₑ⁻¹ A B C
    ≈⟨ pullʳ (Cancellers.cancelʳ (assocₑ-isoʳ A B C)) ⟩
  α⇐ ∘ κ⁻¹ (A ⊗ (B ⊗ C))
    ∎)
κ⁻¹-natural (ρ⇐ᶠ {X}) = ⟺ (begin
  ...
  (ρ⇐ ∘ (κ⁻¹ X ∘ unitʳₑ X)) ∘ unitʳₑ⁻¹ X
    ≈⟨ pullʳ (Cancellers.cancelʳ (unitʳₑ-isoʳ X)) ⟩
  ρ⇐ ∘ κ⁻¹ X
    ∎)
...
loop-trivial-from {X} h loop = begin
  ⟦ h ⟧₁                                               ≈⟨ introʳ (_≅_.isoˡ (strictify X)) ⟩
  ⟦ h ⟧₁ ∘ (κ⁻¹ X ∘ _≅_.from (strictify X))            ≈⟨ pullˡ (κ⁻¹-natural h) ⟩
  (κ⁻¹ X ∘ substₑ (⇒⇒nf h)) ∘ _≅_.from (strictify X)  ≈⟨ elimʳ loop ⟩∘⟨refl ⟩
