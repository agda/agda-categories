...
module Categories.Category.Monoidal.CounitalCopy.Restriction {o ℓ e} {𝒞 : Category o ℓ e} (counitalCopy : CounitalCopy 𝒞) where
  ...
  restriction = record
    ...
    where
      _↓ : ∀ {A B} → A ⇒ B → A ⇒ A
      _↓ {A} {B} f = unitorˡ.from ∘ (δ ⊗₁ id) ∘ (f ⊗₁ id) ∘ Δ
      ...
      pidʳ' {f = f} = begin 
        ...
        unitorˡ.from ∘ (δ ⊗₁ id) ∘ (f ⊗₁ f) ∘ Δ              ≈˘⟨ refl⟩∘⟨ refl⟩∘⟨ natural f ⟩
        unitorˡ.from ∘ (δ ⊗₁ id) ∘ Δ ∘ f                     ≈⟨ refl⟩∘⟨ (pullˡ (sym δ-identityˡ)) ⟩ 
        unitorˡ.from ∘ unitorˡ.to ∘ f                        ≈⟨ cancelˡ unitorˡ.isoʳ ⟩ 
        f                                                    ∎
      ↓-comm' : ∀ {A B C} (f : A ⇒ B) (g : A ⇒ C) → f ↓ ∘ g ↓ ≈ unitorˡ.from ∘ (unitorˡ.from ⊗₁ id) ∘ ((δ ⊗₁ δ) ⊗₁ id) ∘ ((g ⊗₁ f) ⊗₁ id) ∘ (Δ ⊗₁ id) ∘ Δ
      ↓-comm' f g = begin 
      ...
      ↓-denestʳ' {f = f} {g} = begin 
        ...
        unitorˡ.from ∘ (δ ⊗₁ id) ∘ (((((g ∘ unitorˡ.from) ∘ (δ ⊗₁ id)) ∘ (f ⊗₁ id)) ∘ Δ) ⊗₁ ((((id ∘ id) ∘ id) ∘ id) ∘ id)) ∘ Δ
