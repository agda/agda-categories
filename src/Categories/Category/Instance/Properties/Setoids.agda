...
open import Relation.Binary.Construct.Symmetrize as Symmetrize hiding (setoid; trans; sym)
...
Setoids-Cocomplete o ℓ e c ℓ′ {J} F = record
  { initial =
    ...
    { ⊥        = record
      ...
      ; coapex = record
        ...
        ; commute = λ {X} X⇒Y x≈y → disorient (slash (X⇒Y , Π.cong (F₁ X⇒Y) (F₀.sym X x≈y)))
        }
      }
    ; !        = λ {K} →
      ...
      { arr     = record
        { _⟨$⟩_ = to-coapex K
        ; cong  = minimal ⇛-preorder K.N (to-coapex K) (coapex-cong K)
        }
  ...
  where open Functor F
        ...
        _⇛_ : Rel vertex-carrier (o ⊔ ℓ ⊔ c ⊔ ℓ′)
        (X , x) ⇛ (Y , y) = Σ[ f ∈ J [ X , Y ] ] Y [ (F₁ f ⟨$⟩ x) ≈ y ]
        ⇛-preorder : Preorder (o ⊔ c) (o ⊔ c) (o ⊔ ℓ ⊔ c ⊔ ℓ′)
        ⇛-preorder = record
        ...
        to-coapex : ∀ K → vertex-carrier → Setoid.Carrier (Cocone.N K)
        to-coapex K (j , Sj) = K.ψ j ⟨$⟩ Sj
          where module K = Cocone K
        coapex-cong : ∀ K → _⇛_ =[ to-coapex K ]⇒ (Setoid._≈_ (Cocone.N K))
