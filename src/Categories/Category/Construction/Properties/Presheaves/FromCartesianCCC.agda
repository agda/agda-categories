...
module FromCartesian o′ ℓ′ {o ℓ e} {C : Category o ℓ e} (Car : Cartesian C) where
  private
    ...
    open Cartesian Car hiding (_×_)
    open BinaryProducts products hiding (product)
  ...
  CanonicalCCC = record
      ...
      in ntHelper record
        { η       = λ X → record
          { _⟨$⟩_ = λ { (α , x) →
            ...
            in F.₁ CarP.Δ ⟨$⟩ (α.η X ⟨$⟩ x) }
          ; cong  = λ { (eq , eq′) → Π.cong (F.₁ CarP.Δ) (eq eq′) }
          }
        ; commute = λ {X Y} f → λ { {α , x} {β , y} (eq , eq′) →
          let module α = NaturalTransformation α using (η; commute)
              ...
              open SetoidR (F.₀ Y)
          in  begin
            F.₁ CarP.Δ ⟨$⟩ (F.₁ (CarP.first f) ⟨$⟩ (α.η Y ⟨$⟩ (G.₁ f ⟨$⟩ x)))
              ≈⟨ Π.cong (F.₁ CarP.Δ ∙ F.₁ (CarP.first f)) (α.commute f eq′) ⟩
            F.₁ CarP.Δ ∙ F.₁ (CarP.first f) ∙ F.₁ (CarP.second f) ⟨$⟩ (α.η X ⟨$⟩ y)
              ≈⟨ Π.cong (F.₁ CarP.Δ) ([ F ]-resp-∘ CarP.second∘first refl) ⟩
            F.₁ CarP.Δ ⟨$⟩ (F.₁ (f CarP.⁂ f) ⟨$⟩ (α.η X ⟨$⟩ y))
              ≈⟨ [ F ]-resp-∘ CarP.⁂∘Δ refl ⟩
            F.₁ ⟨ f , f ⟩ ⟨$⟩ (α.η X ⟨$⟩ y)
              ≈˘⟨ [ F ]-resp-∘ CarP.Δ∘ (sym (eq (Setoid.refl (G.₀ X)))) ⟩
            F.₁ f ⟨$⟩ (F.₁ CarP.Δ ⟨$⟩ (β.η X ⟨$⟩ y))
              ∎ }
        }
    ; curry        = λ {F G H} α →
            ...
            ; commute = λ {Y Z} f {y z} eq →
              ...
              in begin
                α.η (X × Z) ⟨$⟩ (F.₁ π₁ ⟨$⟩ x , G.₁ π₂ ⟨$⟩ (G.₁ f ⟨$⟩ y))
                  ≈˘⟨ Π.cong (α.η (X × Z)) ( [ F ]-resp-∘ (CarP.π₁∘⁂ CH.○ C.identityˡ) (Setoid.refl (F.₀ X))
                                           , [ G ]-resp-square CarP.π₂∘⁂ (Setoid.refl (G.₀ Y))) ⟩
                α.η (X × Z) ⟨$⟩ (F.₁ (CarP.second f) ∙ F.₁ π₁ ⟨$⟩ x , G.₁ (CarP.second f) ⟨$⟩ (G.₁ π₂ ⟨$⟩ y))
                  ≈⟨ α.commute (CarP.second f) (Setoid.refl (F.₀ (X × Y)) , Π.cong (G.₁ π₂) eq) ⟩
                H.₁ (CarP.second f) ⟨$⟩ (α.η (X × Y) ⟨$⟩ (F.₁ π₁ ⟨$⟩ x , G.₁ π₂ ⟨$⟩ z))
                  ∎
            }
        ...
        ; commute = λ {X Y} f {x y} eq₁ {Z} {z w} eq₂ →
          ...
          in begin
            α.η (Y × Z) ⟨$⟩ (F.₁ π₁ ⟨$⟩ (F.₁ f ⟨$⟩ x) , G.₁ π₂ ⟨$⟩ z)
              ≈˘⟨ Π.cong (α.η _) ( [ F ]-resp-square CarP.π₁∘⁂ (Setoid.refl (F.₀ X))
                                 , [ G ]-resp-∘ (CarP.π₂∘⁂ CH.○ C.identityˡ) (Setoid.refl (G.₀ Z))) ⟩
            α.η (Y × Z) ⟨$⟩ (F.₁ (CarP.first f) ⟨$⟩ (F.₁ π₁ ⟨$⟩ x) , G.₁ (CarP.first f) ⟨$⟩ (G.₁ π₂ ⟨$⟩ z))
              ≈⟨ α.commute (CarP.first f) (Π.cong (F.₁ π₁) eq₁ , Π.cong (G.₁ π₂) eq₂) ⟩
            H.₁ (CarP.first f) ⟨$⟩ (α.η (X × Z) ⟨$⟩ (F.₁ π₁ ⟨$⟩ y , G.₁ π₂ ⟨$⟩ w))
              ∎
        }
    ; eval-comp    = λ {F G H} {α} → λ { {X} {x , y} {z , w} (eq₁ , eq₂) →
      let module F = Functor F
          ...
          open SetoidR (F.₀ X)
      in begin
        F.₁ CarP.Δ ⟨$⟩ (α.η (X × X) ⟨$⟩ (H.₁ π₁ ⟨$⟩ x , G.₁ π₂ ⟨$⟩ y))
          ≈⟨ α.sym-commute CarP.Δ (Π.cong (H.₁ π₁) eq₁ , Π.cong (G.₁ π₂) eq₂) ⟩
        α.η X ⟨$⟩ (H.₁ CarP.Δ ⟨$⟩ (H.F₁ π₁ ⟨$⟩ z) , G.₁ CarP.Δ ⟨$⟩ (G.₁ π₂ ⟨$⟩ w))
          ≈⟨ Π.cong (α.η X) ([ H ]-resp-∘ CarP.project₁ HX.refl , [ G ]-resp-∘ CarP.project₂ GX.refl) ⟩
    ...
    ; curry-unique = λ {F G H} {α β} eq {X} {x y} eq₁ {Y} {z w} eq₂ →
      ...
      in begin
        αXx.η Y ⟨$⟩ z
          ≈˘⟨ G.identity GXY.refl ⟩
        G.₁ C.id ⟨$⟩ (αXx.η Y ⟨$⟩ z)
          ≈˘⟨ [ G ]-resp-∘ (CarP.⁂∘Δ CH.○ CarP.η) GXY.refl ⟩
        G.₁ CarP.Δ ⟨$⟩ (G.F₁ (π₁ CarP.⁂ π₂) ⟨$⟩ (αXx.η Y ⟨$⟩ z))
          ≈˘⟨ Π.cong (G.₁ CarP.Δ) ([ G ]-resp-∘ CarP.second∘first GXY.refl) ⟩
        G.₁ CarP.Δ ⟨$⟩ (G.₁ (CarP.first π₁) ⟨$⟩ (G.₁ (CarP.second π₂) ⟨$⟩ (αXx.η Y ⟨$⟩ z)))
          ≈⟨ Π.cong (G.₁ CarP.Δ ∙ G.₁ (CarP.first π₁)) (αXx.sym-commute π₂ (Setoid.refl (H.₀ Y))) ⟩
        G.₁ CarP.Δ ⟨$⟩ (G.₁ (CarP.first π₁) ⟨$⟩ (αXx.η (X × Y) ⟨$⟩ (H.₁ π₂ ⟨$⟩ z)))
          ≈⟨ Π.cong (G.₁ CarP.Δ) (α.sym-commute π₁ (Setoid.refl (F.₀ X)) (Setoid.refl (H.₀ (X × Y)))) ⟩
        G.₁ CarP.Δ ⟨$⟩ (NaturalTransformation.η (α.η (X × Y) ⟨$⟩ (F.₁ π₁ ⟨$⟩ x)) (X × Y) ⟨$⟩ (H.₁ π₂ ⟨$⟩ z))
