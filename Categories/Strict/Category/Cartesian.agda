...
-- Defines the following properties of a (Strict) Category:
...
module Categories.Strict.Category.Cartesian {o ℓ} (𝒞 : Category o ℓ) where
...
open import Categories.Strict.Object.Terminal 𝒞
open import Categories.Strict.Object.Product 𝒞
open import Categories.Strict.Morphism 𝒞
open import Categories.Strict.Morphism.Reasoning.Core 𝒞
open import Categories.Strict.Category.Monoidal 𝒞
...
record Cartesian : Set (levelOfTerm 𝒞) where
  ...
  monoidal = record
    ...
    ; unitorˡ-commute-to   = λ {_} {_} {f} → begin
        ⟨ ! , id ⟩ ∘ f                                     ≡⟨ ⟨⟩∘ ⟩
        ⟨ ! ∘ f , id  ∘ f ⟩                                ≡⟨ ⟨⟩-cong₂ (sym (!-unique _)) identityˡ ⟩
        ⟨ ! , f ⟩                                          ≡˘⟨ ⟨⟩-cong₂ identityˡ identityʳ ⟩
        ⟨ id ∘ ! , f ∘ id ⟩                                ≡˘⟨ ⟨⟩-cong₂ (pullʳ project₁) (pullʳ project₂) ⟩
        ⟨ (id ∘ π₁) ∘ ⟨ ! , id ⟩ , (f ∘ π₂) ∘ ⟨ ! , id ⟩ ⟩ ≡˘⟨ ⟨⟩∘ ⟩
        ⟨ id ∘ π₁ , f ∘ π₂ ⟩ ∘ ⟨ ! , id ⟩                  ∎ -- let open NaturalIsomorphism ⊤×--id in ⇐.commute _
    ; unitorʳ-commute-from = project₁
    ; unitorʳ-commute-to   = λ {_} {_} {f} → begin
        ⟨ id , ! ⟩ ∘ f                                     ≡⟨ ⟨⟩∘ ⟩
        ⟨ id ∘ f , ! ∘ f ⟩                                 ≡⟨ ⟨⟩-cong₂ identityˡ (sym (!-unique _)) ⟩
        ⟨ f , ! ⟩                                          ≡˘⟨ ⟨⟩-cong₂ identityʳ identityˡ ⟩
        ⟨ f ∘ id , id ∘ ! ⟩                                ≡˘⟨ ⟨⟩-cong₂ (pullʳ project₁) (pullʳ project₂) ⟩
        ⟨ (f ∘ π₁) ∘ ⟨ id , ! ⟩ , (id ∘ π₂) ∘ ⟨ id , ! ⟩ ⟩ ≡˘⟨ ⟨⟩∘ ⟩
        ⟨ f ∘ π₁ , id ∘ π₂ ⟩ ∘ ⟨ id , ! ⟩                  ∎ -- let open NaturalIsomorphism -×⊤-id in ⇐.commute _
    ...
    ; pentagon             = begin
      ...
      ⟨ π₁ ∘ assocˡ ∘ π₁ , assocˡ ∘ ⟨ (π₂ ∘ π₁) ∘ (assocˡ ⁂ id) , π₂ ∘ (assocˡ ⁂ id) ⟩ ⟩
        ≡⟨ ⟨⟩-cong₂ (pullˡ project₁) (refl⟩∘⟨ (⟨⟩-cong₂ (pullʳ project₁) project₂)) ⟩
      ⟨ (π₁ ∘ π₁) ∘ π₁ , assocˡ ∘ ⟨ π₂ ∘ assocˡ ∘ π₁ , id ∘ π₂ ⟩ ⟩
        ≡⟨ ⟨⟩-cong₂ assoc (refl⟩∘⟨ (⟨⟩-cong₂ (pullˡ project₂) identityˡ)) ⟩
