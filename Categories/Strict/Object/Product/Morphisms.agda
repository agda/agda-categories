...
module Categories.Strict.Object.Product.Morphisms {o ℓ} (𝒞 : Category o ℓ) where

open import Function using (flip)
open import Level
open import Relation.Binary.PropositionalEquality using (_≡_; sym; refl; cong₂; trans)

open import Categories.Strict.Object.Product.Core 𝒞
...
[_⇒_]×-cong₂ p₁ p₂ f≡g h≡i =
    Product.⟨⟩-cong₂ p₂ (cong₂ _∘_ f≡g refl) (cong₂ _∘_ h≡i refl)
...
[_⇒_]×∘⟨⟩ {f = f} {g = g} {f′ = f′} {g′ = g′} p₁ p₂ = begin
  ...
  [ p₂ ]⟨ f ∘ p₁.π₁ ∘ p₁.⟨_,_⟩ f′ g′
        , g ∘ p₁.π₂ ∘ p₁.⟨_,_⟩ f′ g′ ⟩                ≡⟨ p₂.⟨⟩-cong₂ (cong₂ _∘_ refl p₁.project₁) (cong₂ _∘_ refl p₁.project₂) ⟩
  [ p₂ ]⟨ f ∘ f′ , g ∘ g′ ⟩                           ∎
  where module p₁ = Product p₁
        module p₂ = Product p₂

[_]⟨⟩∘ : ∀ (p : Product A B) → [ p ]⟨ f , g ⟩ ∘ h ≡ [ p ]⟨ f ∘ h , g ∘ h ⟩
[ p ]⟨⟩∘ = sym (unique (⟺ assoc ○ (project₁ ⟩∘⟨refl)) (⟺ assoc ○ (project₂ ⟩∘⟨refl)))
  where open Product p
