...
module Categories.Strict.Object.Product.Core {o ℓ} (𝒞 : Category o ℓ) where

open import Level
open import Function using (flip; _$_)
open import Relation.Binary.PropositionalEquality using (_≡_; refl)
  renaming (trans to _○_; sym to ⟺)

open import Categories.Strict.Morphism 𝒞
open import Categories.Strict.Morphism.Reasoning 𝒞
...
Associable p₁ p₂ p₃ = record
  ...
  ; unique = λ {_ i f g} pf₁ pf₂ → begin
    p₃ ⟨ π₁ p₁ ∘ f , p₂ ⟨ π₂ p₁ ∘ f , g ⟩ ⟩             ≡⟨ ⟨⟩-cong₂ p₃ (refl⟩∘⟨ (⟺ pf₁))
                                                          (⟨⟩-cong₂ p₂ (refl⟩∘⟨ (⟺ pf₁)) (⟺ pf₂)) ⟩
    p₃ ⟨ π₁ p₁ ∘ p₁ ⟨ π₁ p₃ , π₁ p₂ ∘ π₂ p₃ ⟩ ∘ i
       , p₂ ⟨ π₂ p₁ ∘ p₁ ⟨ π₁ p₃ , π₁ p₂ ∘ π₂ p₃ ⟩ ∘ i
            , (π₂ p₂ ∘ π₂ p₃) ∘ i ⟩ ⟩                   ≡⟨ ⟨⟩-cong₂ p₃ (pullˡ (project₁ p₁))
                                                          (⟨⟩-cong₂ p₂ ((pullˡ (project₂ p₁)) ○ assoc)
                                                                       assoc) ⟩
    p₃ ⟨ π₁ p₃ ∘ i
