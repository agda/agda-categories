...
-- For backwards compatibility with the old functor equality.  Should
-- probably be removed eventually.

module OldEquiv {C : Category o ℓ e} {D : Category o′ ℓ′ e′} where
  private
    module C = Category C
    module D = Category D
  open HId D

  -- The old definition of functor equality

  _≡F′_ : (F G : Functor C D) → Set (ℓ ⊔ o ⊔ o′ ⊔ e′)
  _≡F′_ F G =
    Σ ((X : C.Obj) → Functor.F₀ F X ≡ Functor.F₀ G X)
      (λ eq → ∀ {A B : C.Obj} (f : C [ A , B ]) → subst₂ D._⇒_ (eq A) (eq B) (Functor.F₁ F f) D.≈ Functor.F₁ G f)
  ...
  module _ {F G : Functor C D} where
    ...
    module _ (F≡G : F ≡F G) where
      open _≡F_ F≡G

      eq₁-old : ∀ {X Y} (f : C [ X , Y ]) →
                ≡.subst₂ D._⇒_ (eq₀ X) (eq₀ Y) (F₁ f)  D.≈  G₁ f
      eq₁-old {X} {Y} f = begin
          ≡.subst₂ (D [_,_]) (eq₀ X) (eq₀ Y) (F₁ f)
        ≈⟨ hid-subst₂ (eq₀ X) (eq₀ Y) (F₁ f) ⟩
          hid (eq₀ Y) ∘ (F₁ f ∘ hid (eq₀⁻¹ X))
        ≈˘⟨ pullʳ (eq₁⁻¹ f) ⟩
          (hid (eq₀ Y) ∘ hid (eq₀⁻¹ Y)) ∘ G₁ f
        ≈⟨ elimˡ (hid-symʳ (eq₀ Y)) ⟩
          G₁ f
        ∎
        where
    ...
    fromOld : F ≡F′ G → F ≡F G
    fromOld (eq₀ ,, eq₁-old) = record
      { eq₀ = eq₀
      ; eq₁ = λ {X Y} f → begin
            hid (eq₀ Y) ∘ F₁ f
          ≈⟨ introʳ (hid-symˡ (eq₀ X)) ⟩
            (hid (eq₀ Y) ∘ F₁ f) ∘ (hid (sym $ eq₀ X) ∘ hid (eq₀ X))
          ≈⟨ pullˡ assoc ⟩
            (hid (eq₀ Y) ∘ (F₁ f ∘ hid (sym $ eq₀ X))) ∘ hid (eq₀ X)
          ≈˘⟨ ∘-resp-≈ˡ (hid-subst₂ (eq₀ X) (eq₀ Y) (F₁ f)) ⟩
            ≡.subst₂ (D [_,_]) (eq₀ X) (eq₀ Y) (F₁ f) ∘ hid (eq₀ X)
          ≈⟨ ∘-resp-≈ˡ (eq₁-old f) ⟩
            G₁ f ∘ hid (eq₀ X)
          ∎
      }
