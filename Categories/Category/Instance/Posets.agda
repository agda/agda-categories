...
fun-resp-≈ : ∀ {a₁ a₂ a₃ b₁ b₂ b₃} {A : Poset a₁ a₂ a₃} {B : Poset b₁ b₂ b₃}
             (f : A ⇒-Poset B) → fun f Preserves ₍ A ₎_≈_ ⟶ ₍ B ₎_≈_
fun-resp-≈ {A = A} {B} f x≈y =
  antisym B (monotone f (reflexive A x≈y))
...
Posets c ℓ₁ ℓ₂ = record
  { Obj       = Poset c ℓ₁ ℓ₂
  ; _⇒_       = _⇒-Poset_
  ; _≈_       = λ {A B} f g → ∀ {x} → ₍ B ₎ fun f x ≈ fun g x
  ; id        = id
  ; _∘_       = _∘_
